\subsection{Limites du relationnel dans le monde moderne}

\begin{frame}{Pourquoi apprendre NoSQL ? À quoi ça sert ?}

    Les bases de données relationnelles sont idéales pour les \textbf{données structurées, stables et peu volumineuses}.
    \pause
    Cependant, elles ont des limites dans des contextes où :
    ...
    \begin{figure}[htb]
        ...
        \begin{tikzpicture}
            ...
            \node[inner sep=0pt, label=below:Données non-structurées, right=of big-data, xshift=1cm] (unstructured)
    ...
    \note{
        ...
        \textbf{3V, 5V du Big Data}

        \begin{itemize}
            \item Volume : La quantité de données générées et stockées
            \item Vélocité : La vitesse à laquelle les données sont générées et traitées
            \item Variété : Les différents types de données (structurées, semi-structurées, non structurées)
            \item Véracité : La qualité et la fiabilité des données
            \item Valeur : L'utilité des données pour l'entreprise
        \end{itemize}
...
\begin{frame}{Mais que signifie NoSQL ?}
    ...
    \begin{definitionbox}
        NoSQL signifie \textbf{Not Only SQL} ou \textbf{No SQL}.
    \end{definitionbox}
    ...
    \begin{figure}[htb]
        ...
        \begin{tikzpicture}
            ...
            \node[inner sep=0pt, label=below:Coût, right=of scale, xshift=1cm] (cost)
                {\includegraphics[width=.2\textwidth]{img/icon/cost-cutting.png}};
        \end{tikzpicture}
        }
    \end{figure}
